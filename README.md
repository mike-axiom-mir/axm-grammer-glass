...
Construction Hand v1 is the first bounded source-producing adapter on that path. It binds the exact Kiln, Draft Star and inert packet lineage, gives every grounded universal atom an explicit structural influence receipt, and deterministically renders a self-contained offline `index.html` containing real HTML, CSS and JavaScript. Exact RNG rolls now route across four genuine program architectures: a numeric Lineage Signal, grounded-atom Flow Router, finite-state Orbit and bounded in-memory Receipt Ledger. Program-family and architecture-shape receipts are bound through the plan, artifact and viewer bundle. The same candidate, direction and adapter reproduce identical UTF-8 bytes. Unsupported atom roles stop at `HELD_CONSTRUCTION_RULE_REQUIRED` instead of guessing a translation.
...
The static viewer records the combination identity, replay inputs and grounded receipt digest in an append-only device-local ledger. Invalid, corrupted or wrong-snapshot ledger bytes produce a sealed load HOLD and remain untouched; the viewer cannot silently reinterpret them as empty history or overwrite them during preparation. Export remains available while held, and an explicit user-confirmed reset first stores the exact bytes in a content-addressed local quarantine. See `docs/DISCOVERY_LEDGER_RECOVERY_CONTRACT_V1.md`. Construction runtime receipts enter that history only while their own digest is current and their plan, adapter, artifact, file, request, executor and one-attempt containment fields match the prepared construction exactly; a changed result under a retained digest is rejected rather than resealed as new evidence. This is integrity and lineage validation, not producer authentication. If the snapshot contains exact recorded Interglass lineage, the viewer can bind the existing arm/run controls. Generated snapshots now carry a bounded, source-free Construction Field covering eight deterministic rolls by default (configurable from 1–24). Every covered roll has its own exact plan, program architecture and expected artifact digests; failed requested rolls are retained as named holds rather than silently disappearing. A matching combination can replay the shared renderer, compare the resulting bytes with precomputed digests, show the source as inert text, then require separate **ARM SOURCE** and **RUN SOURCE ONCE** actions in an opaque-origin iframe. Manual or out-of-field probes still stop visibly at the missing adapter. Coverage and diversity counts are not novelty or quality rankings, and “new to this ledger” remains local history only.
